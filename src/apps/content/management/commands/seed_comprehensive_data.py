...
class Command(BaseCommand):
    ...
    def clear_test_data(self):
        ...
        AssetAccessRequest.objects.filter(developer_user__email__contains='@example.').delete()
        ...
        PublishingOrganizationMember.objects.filter(user__email__contains='@example.').delete()
    ...
    def create_minimal_dataset(self):
        ...
        license = self.create_test_license('cc0')
        user = self.create_test_user('minimal@example.com', 'Test', 'User')
        org = self.create_test_organization('Test Organization', user)
        resource = self.create_test_resource('Test Resource', org, license)
        asset = self.create_test_asset('Test Asset', org, license, resource)
    ...
    def create_test_licenses(self):
        ...
        licenses_data = [
            {
                ...
                'permissions': [
                    {'key': 'commercial_use', 'label': 'Commercial use', 'description': 'You may use for commercial purposes'},
                    {'key': 'modification', 'label': 'Modification', 'description': 'You may modify the work'},
                    {'key': 'distribution', 'label': 'Distribution', 'description': 'You may distribute the work'}
                ],
                'conditions': [],
                'limitations': [
                    {'key': 'no_warranty', 'label': 'No warranty', 'description': 'No warranty is provided'}
                ]
            },
            {
                'code': 'cc-by-4.0',
                ...
                'permissions': [
                    {'key': 'commercial_use', 'label': 'Commercial use', 'description': 'Commercial use allowed'},
                    {'key': 'modification', 'label': 'Modification', 'description': 'Modification allowed'},
                    {'key': 'distribution', 'label': 'Distribution', 'description': 'Distribution allowed'}
                ],
                'conditions': [
                    {'key': 'include_copyright', 'label': 'Include copyright', 'description': 'Include copyright notice'},
                    {'key': 'include_license', 'label': 'Include license', 'description': 'Include license text'}
                ],
                'limitations': [
                    {'key': 'no_warranty', 'label': 'No warranty', 'description': 'No warranty provided'}
                ]
            },
            {
                'code': 'mit',
                ...
                'permissions': [
                    ...
                    {'key': 'modification', 'label': 'Modification', 'description': 'Modification allowed'},
                    {'key': 'distribution', 'label': 'Distribution', 'description': 'Distribution allowed'},
                    {'key': 'private_use', 'label': 'Private use', 'description': 'Private use allowed'}
                ],
                'conditions': [
                    {'key': 'include_copyright', 'label': 'Include copyright', 'description': 'Include copyright notice'},
                    {'key': 'include_license', 'label': 'Include license', 'description': 'Include license text'}
                ...
                'limitations': [
                    {'key': 'no_warranty', 'label': 'No warranty', 'description': 'No warranty provided'},
                    {'key': 'no_liability', 'label': 'No liability', 'description': 'No liability assumed'}
                ]
            }
        ]
        ...
        for license_data in licenses_data:
            ...
            if created:
                self.stdout.write(f'  ✅ Created license: {license.name}')
        ...
        for i, org_data in enumerate(organizations_data):
            ...
            if created:
                ...
                if i < len(users):
                    ...
                    self.stdout.write(f'    📝 Added {users[i].get_full_name()} as owner')
    ...
    def create_test_organization(self, name, owner_user):
        ...
        org, created = PublishingOrganization.objects.get_or_create(
            ...
            defaults={
                ...
                'description': f'This is a test organization created for development purposes.',
        ...
        for asset_data in assets_data:
            ...
            if created:
                ...
                if resource_version:
                    ...
                    self.stdout.write(f'    🔗 Linked to {resource.name} v{resource_version.semvar}')
    ...
    def create_test_access_workflow(self, users, assets):
        ...
        for scenario in access_scenarios:
            ...
            if created and scenario['status'] == 'approved':
                ...
                self.stdout.write(f'  ✅ Created access: {user.get_full_name()} → {asset.title}')
    ...
    def create_test_distributions(self, resources):
        ...
        for dist_data in distribution_examples:
            ...
            self.stdout.write(f'  ✅ Created distribution: {resource.name} → {dist_data["format_type"]}')
    ...
    def create_test_usage_events(self, users, assets, resources):
        ...
        event_patterns = [
            # Asset downloads
            {'user_index': 1, 'asset_index': 0, 'usage_kind': 'file_download', 'count': 5},
            {'user_index': 2, 'asset_index': 1, 'usage_kind': 'file_download', 'count': 3},
            {'user_index': 1, 'asset_index': 2, 'usage_kind': 'file_download', 'count': 2},
            ...
            # API access
            {'user_index': 1, 'resource_index': 0, 'usage_kind': 'api_access', 'count': 25},
            {'user_index': 2, 'resource_index': 1, 'usage_kind': 'api_access', 'count': 15}
        ]
