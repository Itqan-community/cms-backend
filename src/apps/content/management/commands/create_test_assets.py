        ...
        for i, asset_data in enumerate(test_assets, 1):
            ...
            try:
                ...
                if created:
                    ...
                    self.stdout.write(f"   ℹ️  Already exists: {resource.id}")
                    existing_count += 1

            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f"   ❌ Error creating {asset_data['title']}: {str(e)}")
                )

        # Summary
        self.stdout.write('\n' + '='*60)
        self.stdout.write(
            self.style.SUCCESS(f'🎉 Test Assets Creation Complete!')
        )
        self.stdout.write(f'📊 Summary:')
        self.stdout.write(f'   • Created: {created_count} new assets')
        self.stdout.write(f'   • Existing: {existing_count} assets')
        self.stdout.write(f'   • Total: {created_count + existing_count} test assets available')
        
        if created_count > 0:
            self.stdout.write('\n🔍 Verify your test data:')
            self.stdout.write('   curl -s "https://develop.api.cms.itqan.dev/api/v1/assets/" | jq .')
