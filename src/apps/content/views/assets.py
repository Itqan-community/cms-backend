...
from .models import Asset, AssetAccessRequest, AssetAccess, UsageEvent
from .serializers import (
    AssetSummarySerializer, AssetDetailSerializer, AccessRequestResponseSerializer
)
...
class AssetListView(APIView):
    ...
    def get(self, request):
        ...
        if publisher_id:
            queryset = queryset.filter(resource__publisher_id=publisher_id)
...
class AssetRequestAccessView(APIView):
    ...
    def post(self, request, asset_id):
        ...
        if not purpose or not intended_use:
            return Response(
                {'error': {'code': 'INVALID_REQUEST', 'message': 'Purpose and intended_use are required'}},
                status=status.HTTP_400_BAD_REQUEST
            )
        
        if intended_use not in ['commercial', 'non-commercial']:
            return Response(
                {'error': {'code': 'INVALID_INTENDED_USE', 'message': 'intended_use must be commercial or non-commercial'}},
                status=status.HTTP_400_BAD_REQUEST
            )
...
class AssetDownloadView(APIView):
    ...
    def get(self, request, asset_id):
        ...
        if not user_access or not user_access.is_active:
            return Response(
                {'error': {'code': 'ACCESS_DENIED', 'message': 'You do not have access to this asset'}},
        ...
        if not download_url:
            return Response(
                {'error': {'code': 'DOWNLOAD_UNAVAILABLE', 'message': 'Download URL not available'}},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
def asset_related(request, asset_id):
    ...
    for related in related_assets:
        related_data.append(AssetSummarySerializer.from_asset_model(related, request))
