...
from .models import License, Asset, Resource
from .serializers import LicenseSummarySerializer, LicenseDetailSerializer
            ...
            if isinstance(terms_data[category], list):
                for item in terms_data[category]:
                    if isinstance(item, dict) and 'title_ar' in item:
                        ...
                        item['description'] = item.get('description_ar', item.get('description', ''))
...
def search_licenses(request):
    ...
    if search_terms:
        queryset = queryset | License.objects.filter(
            ...
            Q(license_terms__icontains=query)
        )
...
def default_license(request):
    ...
    try:
        ...
        if not license_obj:
            return Response(
                {'error': {'code': 'DEFAULT_LICENSE_NOT_FOUND', 'message': 'Default license not configured'}},
                status=status.HTTP_404_NOT_FOUND
            )
