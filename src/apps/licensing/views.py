...
from .serializers import (
    ...
    AccessRequestWorkflowSerializer, BulkAccessRequestActionSerializer
)
from apps.api.permissions import (
    LicensePermission, AccessRequestPermission, IsAdminUser
)
...
class AccessRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        ...
        elif self.action in ['workflow_action', 'start_review', 'revoke']:
        ...
        for access_request in access_requests:
            try:
                if action == 'start_review' and access_request.can_be_reviewed():
                    access_request.start_review(admin_user=admin_user, admin_notes=admin_notes)
                    successful_count += 1
                    ...
                    failed_count += 1
                    errors.append(f"Request {access_request.id}: Cannot {action} in current state")
            except Exception as e:
                failed_count += 1
                errors.append(f"Request {access_request.id}: {str(e)}")
