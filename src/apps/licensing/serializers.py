...
class LicenseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'license_type', 'license_type_display', 'terms',
            'geographic_restrictions', 'usage_restrictions',
            'requires_approval', 'is_effective', 'is_expired',
            'effective_from', 'expires_at', 'days_until_expiry',
            'access_request_count', 'is_active', 'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'resource_title', 'resource_type', 'license_type_display',
            'is_effective', 'is_expired', 'days_until_expiry', 
    ...
    def get_access_request_count(self, obj):
        ...
        return obj.resource.distributions.filter(
    ...
    def validate_geographic_restrictions(self, value):
        ...
        if value and not isinstance(value, dict):
            raise serializers.ValidationError("Geographic restrictions must be a JSON object")
    ...
    def validate_usage_restrictions(self, value):
        ...
        if value and not isinstance(value, dict):
            raise serializers.ValidationError("Usage restrictions must be a JSON object")
        ...
        if value:
            ...
            if 'requires_attribution' in value:
                if not isinstance(value['requires_attribution'], bool):
                    raise serializers.ValidationError("Requires attribution must be boolean")
            
            if 'attribution_text' in value:
                if not isinstance(value['attribution_text'], str):
                    raise serializers.ValidationError("Attribution text must be a string")
...
class AccessRequestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'distribution', 'distribution_info', 'resource_info',
            'status', 'status_display', 'priority', 'priority_display',
            'justification', 'admin_notes', 'rejection_reason', 'rejection_reason_display',
            'approved_by', 'approved_by_name', 'notification_sent',
            'is_expired', 'is_access_valid', 'can_be_reviewed', 'can_be_revoked',
            'requested_at', 'reviewed_at', 'expires_at', 'days_until_expiry',
            'is_active', 'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'requester_name', 'requester_email', 'distribution_info',
            'resource_info', 'approved_by_name', 'status_display', 'is_expired',
            'is_access_valid', 'requested_at', 'reviewed_at', 'days_until_expiry',
    ...
    def validate_status(self, value):
        ...
        valid_statuses = ['pending', 'under_review', 'approved', 'rejected', 'expired', 'revoked']
...
class AccessRequestListSerializer(serializers.ModelSerializer):
    ...
    """
    requester_name = serializers.CharField(source='requester.get_full_name', read_only=True)
    resource_title = serializers.CharField(source='distribution.resource.title', read_only=True)
    ...
    class Meta:
        model = AccessRequest
        fields = [
            'id', 'requester_name', 'resource_title', 'distribution_format',
            'status', 'status_display', 'priority', 'priority_display', 
