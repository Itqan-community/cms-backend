...
class LicenseAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'resource', 'license_type', 'requires_approval', 
        'is_effective_now', 'effective_from', 'expires_at', 'created_at'
    ]
    list_filter = [
        'license_type', 'requires_approval', 'is_active', 
    ...
    is_effective_now.short_description = 'Status'
...
class AccessRequestAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'requester', 'distribution_resource', 'distribution_format', 
        'status', 'requested_at', 'reviewed_at', 'approved_by'
    ]
    list_filter = [
    ...
    search_fields = [
        ...
        'distribution__resource__title', 'justification'
    ]
    ...
    fieldsets = (
        ...
        ('System Info', {
            'fields': ('id', 'is_active', 'requested_at', 'created_at', 'updated_at'),
            'classes': ('collapse',)
        }),
    )
    ...
    reject_requests.short_description = "Reject selected access requests"
    
    def get_queryset(self, request):
        """Optimize queryset with select_related"""
        return super().get_queryset(request).select_related(
            'requester', 'distribution', 'distribution__resource', 
            'distribution__resource__publisher', 'approved_by'
        )