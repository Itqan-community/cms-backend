...
Requires `from collections.abc import Sequence` and `from typing import Any`.

**Do not read the snapshot off the instance.** Touching `instance.template` inside
`from_db` is what makes `Asset.objects.only("id", "name")` — and any `.defer()`
omitting `template` or `mushaf_layout` — die with `RecursionError`: the deferred
attribute lazily refetches, which re-enters `from_db`, which touches the other
deferred attribute, forever.
...
The comparison runs only when both fields were actually loaded, so
`self.template` here can never itself trigger a deferred fetch.
