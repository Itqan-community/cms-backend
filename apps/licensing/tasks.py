...
def send_access_request_notification(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"AccessRequest {access_request_id} not found for notification",
            )
        ...
        if success:
            logger.info(
                f"Sent {notification_type} notification for access request {access_request_id}",
            )
            return {
                "status": "sent",
        ...
        logger.error(
            f"Failed to send {notification_type} notification for access request {access_request_id}",
        )
        return {
            "status": "failed",
            "access_request_id": access_request_id,
        ...
        logger.error(
            f"Error sending notification for access request {access_request_id}: {exc}",
        )
            ...
            for access_request in expiring_requests:
                try:
                    ...
                    if success:
                        total_reminders += 1
                        logger.info(
                            f"Sent {days}-day expiry reminder for access request {access_request.id}",
                        )
                    else:
                        logger.error(
                            f"Failed to send {days}-day expiry reminder for access request {access_request.id}",
                        )

                except Exception as e:
                    logger.error(
                        f"Error sending expiry reminder for access request {access_request.id}: {e}",
                    )
    ...
    try:
        ...
        for access_request in expired_requests:
            try:
                ...
                logger.error(
                    f"Error marking access request {access_request.id} as expired: {e}",
                )
    ...
    try:
        ...
        for access_request in requests_needing_notification:
            try:
                ...
                resent_count += 1
                logger.info(
                    f"Queued retry notification for access request {access_request.id}",
                )

            except Exception as e:
                logger.error(
                    f"Error queuing retry notification for access request {access_request.id}: {e}",
                )
