...
class AccessRequestNotificationService:
    ...
    def __init__(self):
        self.from_email = getattr(
            settings,
            "DEFAULT_FROM_EMAIL",
            "noreply@itqan-cms.com",
        )
        self.admin_emails = getattr(settings, "ADMIN_NOTIFICATION_EMAILS", [])
        self.notification_enabled = getattr(
            settings,
            "ACCESS_REQUEST_NOTIFICATIONS_ENABLED",
            True,
        )

    def send_request_submitted_notification(
        self,
        ...
        try:
            ...
            logger.info(
                f"Sent submission notification for access request {access_request.id}",
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to send submission notification for access request {access_request.id}: {e}",
            )
            return False
    ...
    def send_request_approved_notification(self, access_request: AccessRequest) -> bool:
        ...
        try:
            ...
            access_request.mark_notification_sent()
            logger.info(
                f"Sent approval notification for access request {access_request.id}",
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to send approval notification for access request {access_request.id}: {e}",
            )
            return False
    ...
    def send_request_rejected_notification(self, access_request: AccessRequest) -> bool:
        ...
        try:
            ...
            access_request.mark_notification_sent()
            logger.info(
                f"Sent rejection notification for access request {access_request.id}",
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to send rejection notification for access request {access_request.id}: {e}",
            )
            return False
    ...
    def send_request_revoked_notification(self, access_request: AccessRequest) -> bool:
        ...
        try:
            ...
            access_request.mark_notification_sent()
            logger.info(
                f"Sent revocation notification for access request {access_request.id}",
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to send revocation notification for access request {access_request.id}: {e}",
            )
            return False
    ...
    def send_request_expired_notification(self, access_request: AccessRequest) -> bool:
        ...
        try:
            ...
            access_request.mark_notification_sent()
            logger.info(
                f"Sent expiration notification for access request {access_request.id}",
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to send expiration notification for access request {access_request.id}: {e}",
            )
            return False
    ...
    def send_expiry_reminder_notification(
        ...
        try:
            ...
            logger.info(
                f"Sent expiry reminder for access request {access_request.id} ({days_until_expiry} days)",
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to send expiry reminder for access request {access_request.id}: {e}",
            )
            return False
    ...
    def send_bulk_notifications(
        ...
        for access_request in access_requests:
            try:
                ...
                logger.error(
                    f"Failed to send {notification_type} notification for request {access_request.id}: {e}",
                )
                results["failed"] += 1
...
def send_workflow_notification(access_request: AccessRequest, event_type: str) -> bool:
    ...
    try:
        if event_type == "submitted":
            return notification_service.send_request_submitted_notification(
                access_request,
            )
        if event_type == "approved":
            return notification_service.send_request_approved_notification(
                access_request,
            )
        if event_type == "rejected":
            return notification_service.send_request_rejected_notification(
                access_request,
            )
        if event_type == "revoked":
            return notification_service.send_request_revoked_notification(
                access_request,
            )
        if event_type == "expired":
            return notification_service.send_request_expired_notification(
                access_request,
            )
