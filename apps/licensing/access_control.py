...
class DistributionAccessController:
            ...
            if not access_request:
                ...
                if raise_exception:
                    raise AccessDeniedError(
                        "Access requires approval. Please submit an access request.",
                    )
                return result
    ...
    def _get_active_license(self, resource) -> License | None:
        """Get active license for a resource"""
        try:
            return (
                resource.licenses.filter(
                    is_active=True,
    ...
    def _check_license_restrictions(
        self,
        user: User,
        license_obj: License,
    ) -> dict[str, Any]:
    ...
    def require_distribution_access(self, user: User, distribution) -> dict[str, Any]:
        ...
        """
        try:
            return self.check_distribution_access(
                user,
                distribution,
                ...
                summary.update(
                    {
                        ...
                        "expired_requests": user_requests.filter(
                            status="expired",
                        ).count(),
                    },
                )
