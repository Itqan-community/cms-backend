...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if self.issues_found == 0:
            self.stdout.write(
                self.style.SUCCESS("✅ All data integrity checks passed!"),
            )
        else:
            self.stdout.write(
                self.style.WARNING(
    ...
    def validate_user_data(self):
        ...
        if unverified_users.exists() and self.detailed:
            self.stdout.write(
                f"   i  {unverified_users.count()} users with unverified emails",
            )
        ...
        if incomplete_profiles.exists() and self.detailed:
            self.stdout.write(
                f"   i  {incomplete_profiles.count()} users with incomplete profiles",
            )
    ...
    def validate_organizations(self):
        ...
        if orgs_without_members.exists():
            self.log_issue(
                f"{orgs_without_members.count()} organizations have no members",
            )
        ...
        if orgs_without_owners.exists():
            self.log_issue(
                f"{orgs_without_owners.count()} organizations have no owners",
            )

        # Check for duplicate slugs
        duplicate_slugs = (
            PublishingOrganization.objects.values("slug")
            .annotate(
                count=Count("slug"),
            )
            .filter(count__gt=1)
        ...
        if duplicate_slugs.exists():
            self.log_issue(
                f"{duplicate_slugs.count()} organizations have duplicate slugs",
            )
    ...
    def validate_licenses(self):
        ...
        if default_licenses.count() == 0:
            ...
            self.log_issue(
                ...
                lambda: License.objects.filter(is_default=True)
                .exclude(
        ...
        if licenses_without_permissions.exists():
            self.log_issue(
                f"{licenses_without_permissions.count()} licenses missing permissions",
            )
    ...
    def validate_resources(self):
        ...
        if resources_without_versions.exists():
            self.log_issue(
                f"{resources_without_versions.count()} resources have no versions",
            )
        ...
        if resources_without_latest.exists():
            self.log_issue(
                f"{resources_without_latest.count()} resources have no latest version",
            )
        ...
        if resources_multiple_latest.exists():
            self.log_issue(
                f"{resources_multiple_latest.count()} resources have multiple latest versions",
            )
        ...
        if invalid_semvar.exists():
            self.log_issue(
                f"{invalid_semvar.count()} resource versions have invalid semvar format",
            )
        ...
        if versions_no_storage.exists():
            self.log_issue(
                f"{versions_no_storage.count()} resource versions missing storage URLs",
            )
    ...
    def validate_assets(self):
        ...
        if invalid_category_assets.exists():
            self.log_issue(
                f"{invalid_category_assets.count()} assets have invalid categories",
            )

        # Check for assets without file sizes
        assets_no_size = Asset.objects.filter(
            Q(file_size="") | Q(file_size__isnull=True),
        )

        if assets_no_size.exists():
            self.log_issue(
                f"{assets_no_size.count()} assets missing file size information",
            )
        ...
        if versions_no_url.exists():
            self.log_issue(
                f"{versions_no_url.count()} asset versions missing file URLs",
            )
    ...
    def validate_access_control(self):
        ...
        if approved_no_access.exists():
            self.log_issue(
                f"{approved_no_access.count()} approved requests missing access grants",
            )

        # Check for access grants without requests
        access_no_request = AssetAccess.all_objects.filter(
            asset_access_request__isnull=True,
        )

        if access_no_request.exists():
            self.log_issue(
                f"{access_no_request.count()} access grants without requests",
            )
        ...
        if expired_access.exists() and self.detailed:
            self.stdout.write(
                f"   i  {expired_access.count()} expired access grants (consider cleanup)",
            )

        # Check for duplicate access requests
        duplicate_requests = (
            AssetAccessRequest.objects.values(
                "developer_user",
                "asset",
            )
        ...
        if duplicate_requests.exists():
            self.log_issue(
                f"{duplicate_requests.count()} duplicate access requests found",
            )
    ...
    def validate_usage_events(self):
        ...
        if asset_events_invalid.exists():
            self.log_issue(
                f"{asset_events_invalid.count()} asset events with invalid asset references",
            )
        ...
        if resource_events_invalid.exists():
            self.log_issue(
                f"{resource_events_invalid.count()} resource events with invalid resource references",
            )
        ...
        if conflicting_events.exists():
            self.log_issue(
                f"{conflicting_events.count()} events reference both asset and resource",
            )

        # Check for invalid usage kinds
        valid_usage_kinds = ["view", "file_download", "api_access"]
        invalid_usage_events = UsageEvent.objects.exclude(
            usage_kind__in=valid_usage_kinds,
        )

        if invalid_usage_events.exists():
            self.log_issue(
                f"{invalid_usage_events.count()} events with invalid usage kinds",
            )
    ...
    def validate_distributions(self):
        ...
        if distributions_no_url.exists():
            self.log_issue(
                f"{distributions_no_url.count()} distributions missing endpoint URLs",
            )
        ...
        if invalid_format_distributions.exists():
            self.log_issue(
                f"{invalid_format_distributions.count()} distributions with invalid format types",
            )
    ...
    def validate_relationships(self):
        ...
        inconsistent_assets = Asset.objects.exclude(
            versions__resource_version__resource__publishing_organization=F(
        ...
        if inconsistent_assets.exists():
            self.log_issue(
                f"{inconsistent_assets.count()} assets with inconsistent organization relationships",
            )
        ...
        if inconsistent_licenses.exists() and self.detailed:
            self.stdout.write(
                f"   i  {inconsistent_licenses.count()} assets with different license than resource default",
            )
        ...
        if broken_workflow.exists():
            self.log_issue(
                f"{broken_workflow.count()} access grants with broken workflow relationships",
            )
