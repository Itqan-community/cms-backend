...
class OpenAPIValidator:
    ...
    def _load_openapi_spec(self):
        ...
        try:
            with Path(self.openapi_spec_path).open(encoding="utf-8") as f:
                if self.openapi_spec_path.endswith(
                    ".yaml",
                ) or self.openapi_spec_path.endswith(".yml"):
    ...
    def _convert_openapi_to_json_schema(self, openapi_schema):
        ...
        if "$ref" in json_schema:
            ...
            if ref.startswith("#/components/schemas/"):
                ...
                if schema_name in self.schemas:
                    return self._convert_openapi_to_json_schema(
        ...
        if "items" in json_schema:
            json_schema["items"] = self._convert_openapi_to_json_schema(
                json_schema["items"],
            )
    ...
    def validate_api_endpoint(
        ...
        if status_str not in responses:
            ...
            if "default" not in responses:
                return False, [
                    f"Response {status_code} not defined for {method.upper()} {spec_path}",
                ]
    ...
    def validate_field_types(self, data, schema):
        ...
        for field_name, field_value in data.items():
            ...
            if expected_type == "string" and not isinstance(field_value, str):
                ...
                if field_value is not None:
                    errors.append(
                        f"Field '{field_name}' should be array, got {type(field_value).__name__}",
                    )

            elif expected_type == "object" and not isinstance(field_value, dict):
                if field_value is not None:
                    errors.append(
                        f"Field '{field_name}' should be object, got {type(field_value).__name__}",
                    )
    ...
    def run_comprehensive_validation(self):
        ...
        for endpoint_path, method in endpoints_to_test:
            try:
                ...
                logger.info(
                    f"Validated {method} {endpoint_path}: {'✅' if result['is_valid'] else '❌'}",
                )
...
def run_schema_validation_tests():
    ...
    try:
        ...
        if asset:
            result = validate_serializer_output(
        ...
        logger.info(
            f"✅ Schema Validation Complete: {passed_tests}/{total_tests} tests passed",
        )
