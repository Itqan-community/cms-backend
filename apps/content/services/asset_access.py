...
def approve_request(asset_access_request:AssetAccessRequest, approved_by:User|None, admin_response:str):
...
def request_access(user:User, asset:Asset, purpose:str, intended_use:str, auto_approve=True)->tuple[AssetAccessRequest, AssetAccess|None]:
    existing_request = AssetAccessRequest.objects.filter(
        developer_user=user,
        asset=asset
    ).first()
    ...
    if existing_request:
        if existing_request.status == AssetAccessRequest.StatusChoice.APPROVED:
            ...
            if auto_approve:
                access = approve_request(existing_request,approved_by=None,admin_response="Automatically approved (V1 policy)")
                return existing_request, access
    ...
    access = None
    if auto_approve:
        access = approve_request(request, approved_by=None,admin_response="Automatically approved (V1 policy)")
