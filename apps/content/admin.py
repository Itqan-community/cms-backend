...
from .models import PublishingOrganizationMember
...
class AssetAdmin(admin.ModelAdmin):
    ...
    @admin.display(
        ...
        ordering="access_requests_count",
    )
        ...
        for access_request in queryset.filter(status="pending"):
            try:
                access_request.reject_request(
                    ...
                    reason="Bulk rejection from admin",
                )
                count += 1
            except Exception as e:
                self.message_user(
