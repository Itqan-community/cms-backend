...
class FontCreateTest(BaseTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_create_font_where_valid_data_should_return_201(self):
        # Arrange
        ...
        self.assertEqual(CategoryChoice.FONT, asset.category)
        self.assertEqual(StatusChoice.READY, asset.status)
        self.assertEqual("ar", asset.language)

    def test_create_font_where_publisher_not_found_should_return_404(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_CREATE_FONT)
    ...
    def test_create_font_where_name_missing_should_return_400(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_CREATE_FONT)
    ...
    def test_create_font_where_unauthenticated_should_return_401(self):
        # Arrange
    ...
    def test_create_font_where_only_english_name_should_create_successfully(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_CREATE_FONT)
        ...
        self.assertEqual(201, response.status_code, response.content)
        body = response.json()
        self.assertEqual("English Font", body["name_en"])

    def test_create_font_where_thumbnail_provided_should_upload_and_return_url(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_CREATE_FONT)
    ...
    def test_create_font_where_is_external_true_and_url_present_should_return_201(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_CREATE_FONT)
        ...
        self.assertEqual("https://example.com/font", body["external_url"])

    def test_create_font_where_is_external_true_and_no_url_should_return_400(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_CREATE_FONT)
    ...
    def test_create_font_defaults_visibility_flags_to_false(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_CREATE_FONT)
        ...
        response = self.client.post(
            "/portal/fonts/",
            data={
                "name_en": "Default Visibility Font",
                "license": "CC-BY",
                "language": "ar",
                "publisher_id": self.publisher.id,
                "is_external": False,
    ...
    def test_create_font_sets_visibility_flags(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_CREATE_FONT)
