...
class FontUpdateTest(BaseTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_update_font_where_put_updates_all_fields_should_return_200(self):
        # Arrange
        ...
        response = self.client.put(
            f"/portal/fonts/{self.font.slug}/",
            data={
                "name_ar": "خط محدث",
                "name_en": "Updated Font",
        ...
        self.assertEqual("خط محدث", body["name_ar"])
        self.assertEqual("Updated Font", body["name_en"])
        ...
        # Verify in database
        updated_asset = Asset.objects.get(id=self.font.id)
        self.assertEqual("خط محدث", updated_asset.name_ar)
        self.assertEqual("Updated Font", updated_asset.name_en)
        self.assertEqual("CC-BY", updated_asset.license)
        self.assertEqual("en", updated_asset.language)

    def test_update_font_where_patch_updates_partial_fields_should_return_200(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_UPDATE_FONT)
        ...
        self.assertEqual("خط أصلي", body["name_ar"])
        self.assertEqual("Original description", body["description_en"])
        self.assertEqual("ar", body["language"])

    def test_update_font_where_put_missing_required_field_should_return_400(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_UPDATE_FONT)
    ...
    def test_update_font_where_patch_with_invalid_name_should_return_400(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_UPDATE_FONT)
        ...
        self.assertEqual(400, response.status_code, response.content)
        body = response.json()
        self.assertEqual("font_name_required", body["error_name"])

    def test_update_font_where_not_found_should_return_404(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_UPDATE_FONT)
    ...
    def test_update_font_where_unauthenticated_should_return_401(self):
        # Arrange
    ...
    def test_update_font_where_thumbnail_provided_should_upload_and_return_url(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_UPDATE_FONT)
        ...
        self.font.refresh_from_db()
        self.assertTrue(bool(self.font.thumbnail_url))
        self.assertTrue(self.font.thumbnail_url.name.endswith(".jpg"))

        # Clean up uploaded file
        self.font.thumbnail_url.delete()

    def test_update_font_where_is_external_true_and_url_present_should_return_200(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_UPDATE_FONT)
    ...
    def test_update_font_where_is_external_true_and_no_url_should_return_400(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_UPDATE_FONT)
    ...
    def test_update_font_where_is_external_false_forces_url_null_should_return_200(self):
        # Arrange
        self.authenticate_user(self.user)
        self.give_permission(self.user, PermissionChoice.PORTAL_UPDATE_FONT)
        # first make it external
        self.font.is_external = True
        self.font.external_url = "https://example.com/original"
        self.font.save()
        ...
        # Check db
        self.font.refresh_from_db()
        self.assertFalse(self.font.is_external)
        self.assertIsNone(self.font.external_url)

    def test_update_font_where_user_lacks_permission_should_return_403(self):
        # Arrange
