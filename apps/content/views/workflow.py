...
from apps.api.permissions import IsPublisherOwnerOrReviewer
from apps.api.permissions import IsReviewerOrAdmin
...
class WorkflowViewSet(ViewSet):
    ...
    def dashboard(self, request):
        ...
        for status_code, _ in Resource.WORKFLOW_STATUS_CHOICES:
            stats[status_code] = base_queryset.filter(
                workflow_status=status_code,
            ).count()
    ...
    @action(
        detail=True,
        methods=["post"],
        permission_classes=[IsPublisherOwnerOrReviewer],
    )
    ...
    def history(self, request, pk=None):
        ...
        if not (resource.publisher == request.user or request.user.is_reviewer() or request.user.is_admin()):
            return Response(
