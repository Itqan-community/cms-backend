...
class Searching(SearchingBase):
    ...
    def construct_conditions_for_list(self, search_terms: list[str]) -> dict[str, list[tuple[Callable, str]]]:
        ...
        conditions: dict[str, list[tuple[Callable, str]]] = {field_name: [] for field_name in lookups.keys()}
        for search_term in search_terms:
    ...
    def construct_search_for_list(self) -> dict[str, Callable]:
        ...
        return {
            field_name[1:] if (self.lookup_prefixes_list.get(field_name[0])) else field_name: get_lookup(field_name[0])
