...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Found {len(models_to_unregister)} third-party models to hide from admin:",
        )
        ...
        for model in models_to_unregister:
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Unregistered {model._meta.app_label}.{model._meta.model_name}",
                    ),
                )
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"Already unregistered: {model._meta.app_label}.{model._meta.model_name}",
                    ),
                )
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(
                        f"Error unregistering {model._meta.app_label}.{model._meta.model_name}: {e}",
                    ),
                )

        self.stdout.write(
            self.style.SUCCESS(
                f"Admin cleanup complete. {unregistered_count} models hidden from admin.",
            ),
        )
    ...
    def show_admin_stats(self):
        ...
        self.stdout.write(
            f"Total models: {sum(len(models) for models in stats.values())}",
        )
