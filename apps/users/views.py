...
from apps.users.serializers import AuthResponseSerializer
...
from apps.users.serializers import create_auth_response
from apps.users.serializers import create_error_response
...
class RegisterView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            201: OpenApiResponse(response=AuthResponseSerializer, description="User registered successfully"),
            409: OpenApiResponse(response=ErrorResponseSerializer, description="Email already taken"),
        },
    )
    def post(self, request):
        ...
        return Response(
            {"error": {"code": error_code, "message": error_message, "details": error_details}}, status=status_code
        )
...
class OAuthStartView(APIView):
    ...
    @extend_schema(
        ...
        responses={302: OpenApiResponse(description="Redirect to OAuth provider")},
    )
    def get(self, request, provider=None):
        ...
        try:
            ...
            oauth_url = request.build_absolute_uri(reverse(f"{provider}_login"))
        except Exception as e:
        ...
        return Response({"auth_url": oauth_url, "provider": provider}, status=status.HTTP_200_OK)
...
class OAuthCallbackView(APIView):
    ...
    @extend_schema(
        ...
        responses={200: OpenApiResponse(response=AuthResponseSerializer, description="OAuth login successful")},
    )
    def get(self, request, provider=None):
        ...
        return Response({"message": f"This endpoint is handled by allauth socialaccount views for {provider}"})
            ...
            return Response(
                {
                    ...
                    "profile": {"id": user.id, "profile_completed": user.profile_completed},
                },
                status=status.HTTP_200_OK,
            )

        return Response(
            create_error_response("VALIDATION_ERROR", "Invalid profile data"), status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if response.status_code == 200:
                return Response({"access_token": response.data["access"]}, status=status.HTTP_200_OK)
