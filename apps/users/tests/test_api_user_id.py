...
class PublicApiExternalUserIdHeaderTestCase(BaseTestCase):
    @classmethod
    ...
    def _make_request(self, value):
        ...
        return self.client.get(
            "/recitations/",
            headers={"x-api-key": raw_key, "X-External-User-Id": value},
        )

    def test_public_api_allows_missing_external_user_id_header(self):
        _, raw_key = APIKey.objects.create_key(name=f"User ID Key {uuid4().hex}", user=self.user)

        res = self.client.get("/recitations/", headers={"x-api-key": raw_key})

        self.assertEqual(200, res.status_code, res.content)
        self.assertIsNone(res.wsgi_request.external_user_id)

    def test_public_api_accepts_valid_external_user_id_header(self):
        for valid_value in [
            "user-123",
            "USER_123",
            "abc.def",
            ...
            with self.subTest(valid_value=valid_value):
                ...
                self.assertEqual(200, res.status_code, res.content)
                self.assertEqual(valid_value.strip(), res.wsgi_request.external_user_id)
    ...
    def test_public_api_rejects_empty_and_blank_values_with_422(self):
        for invalid_value in ["", " ", "\t", "\n", "   \n  "]:
            with self.subTest(invalid_value=repr(invalid_value)):
                ...
                self.assertEqual("invalid_external_user_id", res.json()["error_name"])
    ...
    def test_public_api_rejects_obvious_pii_shapes_with_422(self):
        for invalid_value in [
            "user@example.com",
            "1234567890",
            "555-123-4567",
    ...
    def test_same_external_user_id_is_namespaced_by_api_key_owner(self):
        ...
        res_a = self.client.get(
            "/recitations/",
            headers={"x-api-key": raw_key_a, "X-External-User-Id": "same-user-id"},
        )
        res_b = self.client.get(
            "/recitations/",
            headers={"x-api-key": raw_key_b, "X-External-User-Id": "same-user-id"},
        )

        self.assertEqual("same-user-id", res_a.wsgi_request.external_user_id)
        self.assertEqual("same-user-id", res_b.wsgi_request.external_user_id)
