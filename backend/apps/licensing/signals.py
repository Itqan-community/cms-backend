...
def access_request_post_save(sender, instance, created, **kwargs):
    ...
    try:
        ...
        if getattr(settings, 'CELERY_TASK_ALWAYS_EAGER', False):
            logger.debug(f"Skipping notification for access request {instance.id} (Celery eager mode)")
            return
        ...
        if notification_type:
            ...
            logger.info(f"Queued {notification_type} notification for access request {instance.id}")
    
    except Exception as e:
        logger.error(f"Failed to queue notification for access request {instance.id}: {e}")
